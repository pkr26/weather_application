 ...
 * 4xx and network errors surfaced immediately) and both take the
 * per-request abort scope, so a hung-up autocomplete client cancels its
 * in-flight geocode instead of burning it to completion.
 */
export class GeocodingClient {
  constructor(private readonly config: Config) {}

  async search(query: string, count = 12, signal?: AbortSignal): Promise<unknown> {
   ...
   */
  async reverse(latitude: number, longitude: number, signal?: AbortSignal): Promise<unknown> {
